...
class Batch {
  ...
  #traverse_effect_tree(root, effects, render_effects) {
    ...
    while (effect2 !== null) {
      ...
      if (async_mode_flag && pending_boundary === null && (flags & BOUNDARY_EFFECT) !== 0 && effect2.b?.is_pending) {
...
function flatten(blockers, sync, async2, fn) {
  ...
  if (async2.length === 0 && pending.length === 0) {
    fn(sync.map(d));
    return;
  }
  ...
  function run2() {
    restore();
    Promise.all(async2.map((expression) => /* @__PURE__ */ async_derived(expression))).then((result) => finish([...sync.map(d), ...result])).catch((error) => invoke_error_boundary(error, parent));
  }
  if (blocker_promise) {
